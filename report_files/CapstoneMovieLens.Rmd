...
To get a better understanding of edx, let's separate the feature genres to see how many unique genres there are
To optimize the computation we will use an auxiliary dataset call movie_genre

```{r edx_genre, eval=FALSE, include=FALSE}
# first 10 obs, there might be more than one genre per movie
...
# Listing genres
unique(edx_genre$genre) %>% 
  kable(booktabs = T) %>%
  kable_styling( "latex", latex_options = "striped", full_width = "F", position = "left")
```

Let's see per each genre the number of ratings, movies and average rating
```{r genre, eval=FALSE, include=FALSE}
...
Keep in mind that one movie can belong to more than one genre,
We can identify an outlier as there is one movie with no genre and only 7 ratings
...
```{r model_4_choose_lambda, eval=FALSE, include=FALSE}
...
```{r model_4, eval=FALSE, include=FALSE}
...
```{r model_5_choose_lambda, eval=FALSE, include=FALSE}
lambdas <- seq(0, 10, 0.25)
mu_hat <- mean(edx$rating) 
...
```{r model_5, eval=FALSE, include=FALSE}
# Applying lambda on validation set
b_i <- edx %>%
  group_by(movieId) %>%
  summarize(b_i = sum(rating - mu_hat)/(n()+lambda))
